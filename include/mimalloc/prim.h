...
extern mi_decl_thread size_t _mi_tdi_index;
extern mi_decl_thread mi_heap_t* _mi_tdi_heaps[MAX_TDI_HEAPS];

static inline mi_heap_t* mi_prim_get_tdi_heap(void){
  mi_assert_internal(_mi_tdi_index < MAX_TDI_HEAPS);
  mi_heap_t* heap = _mi_tdi_heaps[_mi_tdi_index];
  if(heap == NULL){
    heap = _mi_tdi_heaps[_mi_tdi_index] = mi_heap_new();
  }
  return heap;
}
